...
import { transform, transformImports, transformIcons, transformStripTypes, createTransformInjectStyles } from "../../utils/transformers/index.js";
...
async function runUpdate(cwd: string, config: cliConfig.ResolvedConfig, options: UpdateOptions) {
	...
	for (const item of payload) {
		...
		tasks.push({
			...
			async task() {
				const registryStyle = await registry.getRegistryStyle(registryUrl, config.designSystem.style);
